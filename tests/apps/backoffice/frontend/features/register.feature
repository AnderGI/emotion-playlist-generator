Feature: Register Page
  ...
  Scenario: Check the register page redirection when JWT is found
    Given I send a GET request to "/dashboard" with a jwt cookie appended "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c"
    Then the response status code should be 200
    Then the response header "content-type" should be "text/html; charset=utf-8"
    Then the response text should contain "<h1>Upload Image Page</h1>"
    Then the response text should contain '<form action="/upload" method="post" enctype="multipart/form-data">'
    Then the response text should contain '<input type="file" id="image-upload" name="image" accept="image/png" required>'
    Then the response text should contain '<button type="submit">Upload</button>'
