...
export default class RabbitMqConsumeConfigurer {
		...
		await Promise.all(
			queuesToSubscriber.map(queueToBinding =>
				this.channel.consume(
					...
					this._consume(queueToBinding.subscriber, this.channel, this.domainEventJsonDeserializer)
				)
			)
		);
	}
	...
	private _consume(
		...
		return function (msg: amqplib.ConsumeMessage | null) {
			if (msg !== null) {
				subscriber.on(domainEventJsonDeserializer.deserialize(msg.content.toString()));
				channel.ack(msg);
			}
		};
	}
